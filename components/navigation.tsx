    ...
    return(
        <>
            ...
            <div ref={menuRef} className="absolute m-2">
                <button className="block md:hidden cursor-pointer p-1 focus:bg-black focus:text-white focus:rounded-sm" type="button" onClick={() => setIsOpen(!isOpen)}>
                    <Menu size={28}/>
                </button>
                ...
                {isOpen && (
                    <div className="relative top-2 -left-3 border-r border-y bg-white shadow-lg rounded-md p-4">
                        <ul>
                            {menuItems.map((item) =>
                            <li key= {item.name}>
                                <Link href={item.href} className="flex items-center py-1 px-2 rounded-md focus:outline-none focus:bg-[rgb(117,73,148)] focus:text-white">
            ...
            {/* Normal Navigation */}
            <nav className="hidden md:flex flex-col w-[200px] min-h-screen border-r border-gray-300">
