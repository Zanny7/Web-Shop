...
export default function Table({ products, total, page, pages }: Props) {
  ...
  const getStatus = (status?: string) => {
    if (status === "Out of Stock") return <span className="text-danger font-medium">Out of stock</span>;
    if (status === "Low Stock") return <span className="text-warning font-medium">Low stock</span>;
						...
						<tbody className="divide-y divide-gray-200">
							{products.map((product) => (
								...
								>
											...
											<div>
												{editingId === product.id ? (
													<input
														...
														onChange={handleChange}
														className="border rounded px-2 py-1 text-sm w-full max-w-full"
													/>
												) : (
													<p className="font-medium text-gray-900">
														{product.title}
												...
												{editingId === product.id ? (
													<input
														...
														onChange={handleChange}
														className="border rounded px-2 py-1 text-xs w-full max-w-full mt-1"
													/>
												) : (
									...
									<td className="px-6 py-5">
										{getStatus(product.availabilityStatus)}
									</td>
